...
function getParam(name) {
	if ("URLSearchParams" in window) {
		...
		// Polyfill for IE11
		var params = new RegExp("[?&]" + name + "=([^&#]*)").exec(
		...
		el.addEventListener("change", function (event) {
			...
			variants.filter(function (v) {
				if (
					...
					document
						.querySelectorAll(
							'input[type="checkbox"].js--variant-id'
						)
						.forEach(function (el) {
							el.value = v.id;
					...
					document
						.querySelectorAll(".js--variant-price")
						.forEach(function (el) {
					...
					if (v.compare_at_price > v.price) {
						document
							.querySelectorAll(".js--variant-compareatprice")
							.forEach(function (el) {
								el.innerText = Shopify.formatMoney(
									v.compare_at_price
								);
							});
					} else {
					...
					// Update SKU
					document
						.querySelectorAll(".js--variant-sku")
						.forEach(function (el) {
					...
					if (v.available === false) {
						...
						document.querySelector("#js--addtocart").innerText =
							"Unavailable";
					} else {
						document.querySelector(
							"#js--addtocart"
					...
					var imageIndex = document
						.querySelector(
							'.product-image-item[data-imageid="' +
								imageId +
								'"]'
						)
...
function checkVariants() {
	...
	if ($this !== undefined) {
		...
		document
			.querySelectorAll(".js--variant-option")
			.forEach(function (input) {
				if (input.name != $this.name) {
					if (
						optionGroups[input.name].includes(input.value) == false
					) {
						input.disabled = true;
						input.checked = false;
					} else {
	...
	document.querySelectorAll(".js--variant-options").forEach(function (group) {
		...
		let checkedOptions = group.querySelectorAll(
			".js--variant-option:checked"
		).length;
		if (checkedOptions == 0) {
			firstAvailable = group.querySelectorAll(
				".js--variant-option:not(:disabled)"
			)[0];
		}
